...
const Myproduct = ({ FilteringProducts }) => {
  ...
  useEffect(() => {
    ...
    const Fetchproducts = async () => {
      const products = await fetch("http://localhost:3002/api/allproducts").then((res) => res.json()).then((val) => {
 ...
 //for deleting products 
  const handleDelete=async(phonenumber)=>{
    ...
    const DeleteData=await fetch("http://localhost:3002/api/Deleteproducts",{
      method:"DELETE",
      headers:{
            ...
            return <div className='col-sm-6 mt-5 ' key={index}>
              <div className='card shadow  bg-dark text-center' style={{  }}>
                ...
                <div className='card-body' >
                  <img src={`http://localhost:3002/images/${val.image}`} alt='image is here' className='card-img' style={{ height: "200px" }} />
